...
namespace UnitMag.AODMag
{
    public partial class ucAODData : UserControl
    {
        ...
        public void RefreshContent()
        {
            ...
            string strsqldep = "";
            strsqldep += "select a.materialid,b.heatid,b.weight,b.depart_weight,b.irontemp,c.c,c.si,c.mn,c.p,c.s,c.cr,c.ni from materialposition a ,";
            strsqldep += "cdep_FEED_data b,cqa_lab_element c  ,";
            strsqldep += "(SELECT   heatid, MAX (report_date) AS report_date FROM cqa_lab_element where SAMPLE_ADDRESS='T2' GROUP BY heatid) d ";

            dsArrivleCCMSteel.SourceMethod = strsqldep + " where 1 = 2";
            strsqldep += "where  a.position='S25P01' and a.MATERIALID=b.MATERIALID(+) and b.HEATID=c.heatid(+) and c.REPORT_DATE=d.REPORT_DATE ";
